...
describe('Smart Cart - Shopping List Management', () => {
  ...
  it('should calculate shopping list totals', () => {
    const items = [
      { quantity: 2, cost: 3.50 },
      { quantity: 1, cost: 8.99 },
      { quantity: 3, cost: 2.50 }
    ]
...
describe('Smart Cart - Analytics', () => {
  ...
  it('should track shopping frequency', () => {
    ...
    const avgFrequency = transactions.length > 1
      ? Math.round((transactions.length - 1) / (
          (transactions[0].date.getTime() - transactions[transactions.length - 1].date.getTime())
          / (1000 * 60 * 60 * 24)
        ))
      : 0
